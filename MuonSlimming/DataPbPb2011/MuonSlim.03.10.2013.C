...
int MuonSlim::HIMuonSelector(int i) {
	...
	if( fabs(mu_muid_id_d0_exPV[i])<6)			value++; else return value;	//7
	if( fabs(mu_muid_id_z0_exPV[i]<6))			value++; else return value;	//8
	if( fabs(1./mu_muid_id_qoverp_exPV[i]/1000.) > 3.0)		value++; else return value;	//9
  ...
  for (int i=0; i<entries; i++){
	...
	if(EFtrig1||EFtrig2||EFtrig3||EFtrig4||EFtrig5){
		...
		&& (EF_mu10_MSonly_EFFS_L1ZDC_Matched20[j]==1||EF_mu10_MSonly_EFFS_L1TE10_Matched20[j]==1
			||EF_mu10_MSonly_EFFS_L1TE20_Matched20[j]==1||EF_trigmuonef_EF_mu4_MSonly_L1TE50_Matched20[j]==1||EF_trigmuonef_EF_mu4_L1VTE50_Matched20[j]==1) 
