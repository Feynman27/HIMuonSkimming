...
class MuonSlim {
	...
	public:
		...
		int SlimHISingleMuon(TString inputFiles,bool isMB, bool isHP);	
		int ppMuonSelector(int index);
		int HICombinedMuonSelectorHP(int index);
		int HICombinedMuonSelectorMB(int index);
	...
	private:
		...
		int mc_mu_gen_mothertype[30],mc_mu_gen_type[30];
		int mc_nu_n;
		...
		int Mymc_gamma_gen_type[30], Mymc_gamma_gen_mothertype[30];
		...
		float pt_tru[30], E_tru[30], eta_tru[30], phi_tru[30]; 
		...
		float phi_munu[30] ;
		float phi_munuSm[30] ;
		float caloMETNt,caloMExNt,caloMEyNt,caloMETPhiNt,caloSumEtNt ;
		float nu_ptNt ;
		float nu_pxNt ;
		float nu_pyNt ;
		float nu_pzNt ;
		...
		int EF_mu4_L1VTE50_Matched20[30],EF_mu4_MSonly_L1TE50_Matched20[30]; 
		int L1Matched[30];
