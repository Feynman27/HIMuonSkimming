...
void SmearingClass::Event() {
  ...
  if (Tdet==DET_CB) { smearCB=ICombine(smearMS,smearID); }
  ErrorMatrix();
}
...
double SmearingClass::Combine(double SmearMS, 
  ...
  double SigmaMS=pow(
		     pow(MC_p0_MS[detRegion]/ptms*GeV,2)+
		     pow(MC_p1_MS[detRegion],2)+
		     pow(MC_p2_MS[detRegion]*ptms/GeV,2),0.5);
  ...
  double SigmaID=pow(					      
  		     pow(MC_p1_ID[detRegion],2)+	      
  		     pow(MC_p2_ID[detRegion]*ptid/GeV,2),0.5);
  ...
  if (detRegion==3&&useTan2) {
    ...
    SigmaID=pow(
    		pow(MC_p1_ID[detRegion],2)+
    		pow(MC_p2_ID_TAN[detRegion]*ptid/GeV*sinh(eta)*sinh(eta),2),0.5);
...
double SmearingClass::ScaleMS(double pt) const{
  ...
  return scaleBins.empty() ? pt : ScaleApply(pt,S1_MS[scaleRegion],S2_MS[scaleRegion]);
}
...
double SmearingClass::ScaleID(double pt) const{
  ...
  return scaleBins.empty() ? pt : ScaleApply(pt,S1_ID[scaleRegion],S2_ID[scaleRegion]);
}
...
double SmearingClass::ScaleCB(double pt) const{
  ...
  const double SCB = (detRegion<0 || detRegion>3) ? 1.0 : scale_CB[detRegion];
  if(ScaleType!=""){
    if(unsigned(scaleRegion)>=S1_CB.size() || unsigned(scaleRegion)>=S2_CB.size()){
      std::cout<<"SmearingClass ERROR:: "<<scaleRegion<<" length of scaleRegion1: "<<S1_CB.size()<<std::endl;
      std::cout<<"SmearingClass ERROR:: "<<scaleRegion<<" length of scaleRegion2: "<<S2_CB.size()<<std::endl;
      return -1;
    }
  }
  return scaleBins.empty() ? ScaleApply(pt,0,0,SCB) : ScaleApply(pt,S1_CB[scaleRegion],S2_CB[scaleRegion],SCB);
}
...
SYSTTYPE SmearingClass::SelectFun(std::string fun){
  ...
  else if(fun=="SCALEKDOWN") { Tfun=SCALEC_DOWN; }
  return Tfun;
}
...
void SmearingClass::PTVar(double &PtMS,
  ...
  if (Tfun==ID_UP) { IDUP(PtMS,PtID,PtCB); return; }
    ...
    if(Tscale==SCALE_KC || Tscale==SCALE_K) {
      ...
      const double SCB = (detRegion<0 || detRegion>3) ? 1.0 : scale_CB[detRegion] + s*scaleSyst_CB[detRegion];
      PtCB = ScaleApply(pTCB(),0,0,SCB);
      return;
    }
