...
int main() {
	...
	*/
	...
	//Set the truth branches to be stored
	//in the output tree
	TruthBranch *tru = new TruthBranch;
	if(doTruth) tru->Set_Truth_Branches(MyTree);
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++){
		...
		if(!event) continue;
	 	...
	 	if(!doHistosQuick){	
    			if(doTruth) {tru->Fill_Truth_Branches();}
