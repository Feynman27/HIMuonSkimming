...
class MuonBranch : public BranchBase{
   public:
	...
	int    isGood_Ricard( int i);
	int    isGoodDiMuon( int i, int j);
	void  diMuonMass();
	void  MuonMass();
	void  IDprobes();
	void  MSprobes();
  ...
  private:
   ...
   Float_t   Mymuid_invMass[MAXMUONS]; 
   Float_t   Mymuid_massCB[MAXMUONS*MAXMUONS]; 
   Float_t   Mymuid_massCBSS[MAXMUONS*MAXMUONS]; 
   Float_t   Mymuid_massCBOS[MAXMUONS*MAXMUONS]; 
   std::vector<std::vector<float> > *Vecsinglemuid_massCBOS ;
   std::vector<std::vector<int> > *IDprobe ;
   std::vector<std::vector<int> > *MSprobe ;
   std::vector<float>  Vecsinglemuid_massCBOS_one ;
   ...
   //Int_t     Mytrig_EF_trigmuonef_EF_mu6_L1VTE50[MAXMUONS];
   //Int_t     Mytrig_EF_trigmuonef_EF_mu6_MSonly_L1TE50[MAXMUONS];
