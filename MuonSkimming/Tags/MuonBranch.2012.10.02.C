...
/////////////////////////////////////////////
///THOMAS JET INFORMATION FOR MUON-JET TAGGING
/////////////////////////////////////////////
	...
	for (int itag=0; itag<mu_muid_n;++itag){
		...
		//find the SA probe muon
			...
			for (int reco_track=0;reco_track<trk_n;reco_track++){
          		//	if(reco_track==0&&jentry%1000==0) std::cout << "pt:" << lvSum3000.Perp() << " Theta: " << lvSum3000.Theta() << " Phi: " << lvSum3000.Phi() << endl;
          			...
          			float ptTrkCut = trk_pt->at(reco_track) ;
           			if (
...
//----------------------------------------------------------------------
  ...
  void MuonBranch::IDprobes() {
	...
	for (int reco_track=0;reco_track<trk_n;reco_track++){
    	...
    	TLorentzVector vTrk; vTrk.SetPtEtaPhiM(trk_pt->at(reco_track),trk_eta -> at(reco_track),trk_phi_wrtPV -> at(reco_track),0.1);
		...
		float TPcharge = mu_muid_charge->at(i)*trk_qoverp_wrtPV->at(reco_track) ;
      		...
      		TLorentzVector vMu; vMu.SetPtEtaPhiM(mu_muid_pt->at(i),mu_muid_eta->at(i),mu_muid_phi->at(i),0.1);
