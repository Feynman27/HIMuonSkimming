...
std::cout << "setting muon branches to be stored in tree. " << std::endl;
 ...
 if(isPbP) {
	...
	//Trigger Branches for matching EF items 
        for(int itrig = 0; itrig<N_EF_MATCH_TRIG_pPb; itrig++){
	     std::string name1 = "trig_EF_trigmuonef_" ; 
	     name1+=EF_Trig_Names_pPb[itrig];
	     std::string br_string=name1 +"[trig_EF_trigmuonef_n]/I";
	     //std::cout << br_string << std::endl;
	     myTree->Branch(name1.c_str() ,  &EF_TrigMuon_pPb[itrig]  ,  br_string.c_str());
	}
 }
		...
		//find the SA probe muon
			...
			for (int reco_track=0;reco_track<trk_n;reco_track++){
				...
				///match the MS probe to a high quality ID track
		  		float matchCharge; 
				if(!isPbP) matchCharge = mu_muid_charge->at(iprobe)*trk_qoverp_wrtPV->at(reco_track) ;
				else matchCharge = mu_muid_charge->at(iprobe)*trk_qoverp->at(reco_track) ;
		 		float dPhiMatch = trk_phi_wrtPV->at(reco_track) - mu_muid_phi->at(iprobe) ;
		  		float etaID; 
				if(!isPbP) etaID = -1.*TMath::Log(TMath::Tan(trk_theta_wrtPV->at(reco_track)/2.)) ; 
				else etaID = -1.*TMath::Log(TMath::Tan(trk_theta->at(reco_track)/2.)) ;
	...
	for (int reco_track=0;reco_track<trk_n;reco_track++){
	  ...
	  if(!isPbP){
		...
		if(!isPbP){ 
			...
			TPcharge = mu_muid_charge->at(i)*trk_qoverp->at(reco_track) ;
		}
		...
		for (int j=0; j<mu_muid_n;++j){
		  ...
		  float matchCharge ;
		  if(!isPbP) matchCharge = mu_muid_charge->at(j)*trk_qoverp_wrtPV->at(reco_track) ;
		  else matchCharge = mu_muid_charge->at(j)*trk_qoverp->at(reco_track) ;
		  float dPhiMatch ;
...
void MuonBranch::Fill_Muon_Branches(){
   ...
   //if(trig_EF_trigmuonef_n!=0) std::cout << "trig_EF_trigmuonef_n: " << trig_EF_trigmuonef_n << std::endl;
   if(isPbP){

      //EF online muon trigger Info
      for(int itrig=0;itrig<N_EF_MATCH_TRIG_pPb;itrig++){
	 for(int imu=0;imu<trig_EF_trigmuonef_n;imu++){
	   //std::cout << itrig << ":" << imu << std::endl;
	   //std::cout << "elements in vector = "<< EF_TrigMuon_Reader_pPb[itrig]->size() << std::endl;
	   EF_TrigMuon_pPb[itrig][imu] = EF_TrigMuon_Reader_pPb[itrig]->at(imu) ;
	   //if(EF_TrigMuon_Reader_pPb[itrig]->at(imu)!=0)std::cout << EF_TrigMuon_Reader_pPb[itrig]->at(imu) << " =? " << EF_TrigMuon_pPb[itrig][imu] << std::endl;
	   //std::cout << EF_TrigMuon_Reader_pPb[itrig]->at(imu) << " =? " << EF_TrigMuon_pPb[itrig][imu] << std::endl;
	  }
	}
