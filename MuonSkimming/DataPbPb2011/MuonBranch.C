...
MuonBranch::MuonBranch(){
	...
	//IDprobe = 0;
	//MSprobe = 0;
	hConeTrkMuon = new TH1F("hConeTrkMuon","hConeTrkMuon",1000,0.0,1.0);

}

//----------------------------------------------------------------------
double MuonBranch::getInvMass(int imu1, int imu2){

	TLorentzVector vMu1,vMu2, vSum;

	double px1 = mu_muid_px->at(imu1);
	double py1 = mu_muid_py->at(imu1);
	double pz1 = mu_muid_pz->at(imu1);
	double e1 = mu_muid_E->at(imu1);

	double px2 = mu_muid_px->at(imu2);
	double py2 = mu_muid_py->at(imu2);
	double pz2 = mu_muid_pz->at(imu2);
	double e2 = mu_muid_E->at(imu2);

	vMu1.SetPtEtaPhiE(px1,py1,pz1,e1);
	vMu2.SetPtEtaPhiE(px2,py2,pz2,e2);
	vSum = vMu1+vMu2;
	return vSum.M();
}
//----------------------------------------------------------------------
...
std::cout << "setting muon branches to be stored in tree. " << std::endl;
 ...
 myTree->Branch("mu_muid_npairs",	&mu_muid_npairs,"mu_muid_npairs/I");
 ...
 myTree->Branch("mu_muid_invMassOS",	&mu_muid_invMassOS,	"mu_muid_invMassOS[mu_muid_npairs]/D");
 myTree->Branch("mu_muid_invMassSS",	&mu_muid_invMassSS,	"mu_muid_invMassSS[mu_muid_npairs]/D");
...
// myTree->Branch("IDprobe","vector<vector<int> >",      &IDprobe);
 myTree->Branch("mu_muid_IDTP",&mu_muid_IDTP,      "mu_muid_IDTP[mu_muid_n]/I");
...
//----------------------------------------------------------------------
///fraction of stand-alone muons that can be matched to an ID track
///returns true if probe finds an ID track
  bool MuonBranch::recoIDTP(int itag) {
	...
	//for (int itag=0; itag<mu_muid_n;++itag){
		...
		///find the tag muon (must be a combined muon)
		if(!mu_muid_isCombinedMuon->at(itag)) return hasID;
		float isolationMuTag = mu_muid_ptcone20ID3[itag]/mu_muid_pt->at(itag);
		float muz0Tag = mu_muid_z0_exPV->at(itag);
		float muEtaTag = mu_muid_eta->at(itag);
		if(!isolationMuTag<0.1||fabs(muz0Tag)>=10.0||fabs(muEtaTag)>2.4) return hasID;
      		//int   muQuality = isGoodMuon[itag];

		//match the cb tag muon to a SA probe muon
    		for (int iprobe=0; iprobe<mu_muid_n;++iprobe){

          		//MSprobe_one.clear();
			if(!mu_muid_isStandAloneMuon->at(iprobe)) continue;

			float isolationMuProbe = mu_muid_ptcone20ID3[iprobe]/mu_muid_pt->at(iprobe);
			float muz0Probe = mu_muid_z0_exPV->at(iprobe);
			float muEtaProbe = mu_muid_eta->at(iprobe);
			if(!isolationMuProbe<0.1||fabs(muz0Probe)>=10.0||fabs(muEtaProbe)>2.5) continue;

			///calculate difference of TP pair z0,phi
                	float dz0TP = fabs(mu_muid_z0_exPV->at(itag)- mu_muid_z0_exPV->at(iprobe)) ;
			...
			///make sure the pair meets Z criteria
      			if(mu_muid_pt->at(itag)<20.||mu_muid_pt->at(iprobe)<20.||dz0TP>=3.0||TPcharge>0.||dPhiTP<=2.0) continue;
			...
			*/
      			...
      			///calculate invariant masss of mu tag + probe pair
			float pxT = mu_muid_px->at(itag), pyT = mu_muid_py->at(itag), pzT = mu_muid_pz->at(itag), enT = mu_muid_E->at(itag);
			float pxP = mu_muid_px->at(iprobe), pyP = mu_muid_py->at(iprobe), pzP = mu_muid_pz->at(iprobe), enP = mu_muid_E->at(iprobe);
			TLorentzVector vMuTag ; vMuTag.SetPxPyPzE(pxT,pyT,pzT,enT); 
			TLorentzVector vMuProbe ; vMuProbe.SetPxPyPzE(pxP,pyP,pzP,enP); 
      			TLorentzVector vAdd ; vAdd = vMuTag+vMuProbe;
      			float invMassTemp = vAdd.M();
			///use GeV
			invMassTemp/=1000.0;

			//compare it to the truth Z mass
      			float Zdiff = fabs(invMassTemp - 91.187) ;
			if(Zdiff>=10.0) continue ;

			///now that we have a probe, match it to an ID track 
			///to get ID efficiency

			///use HI tight track requirements
			for (int reco_track=0;reco_track<trk_n;reco_track++){
				...
				///match charge
		  		float matchCharge; 
				float trackCharge = trk_pt->at(reco_track)*TMath::CosH(trk_eta->at(reco_track))*trk_qoverp_wrtPV->at(reco_track);
				matchCharge = mu_muid_charge->at(iprobe)*trackCharge ;
				///match in eta-phi
				...
				float dEtaMatch = etaID - etaMu ;
		  		float dRMatch = TMath::Sqrt(TMath::Power(dPhiMatch,2)+TMath::Power(dEtaMatch,2));

                  		if(dRMatch<=0.05&&matchCharge>0.&&trkQuality==1) hasID = true; 
				//MSprobe_one.push_back(1);
...
///NOT READY FOR USE YET!!!!!!!!!!!!
  bool MuonBranch::recoMSTP(int itag) {
	...
	for (int reco_track=0;reco_track<trk_n;reco_track++){
	  ...
	  float ed0 = -9999.;
    	...
    	TLorentzVector vTrk ; 
	vTrk.SetPtEtaPhiM(trk_pt->at(reco_track)/1000.,trk_eta -> at(reco_track),trk_phi_wrtPV -> at(reco_track),0.1);
...
//     Mytrig_EF_trigmuonef_EF_mu6_L1VTE50[i] = trig_EF_trigmuonef_EF_mu6_L1VTE50->at(i); 
       ...
       if((RunNumber!=193211&&RunNumber!=194079)) Mytrig_EF_trigmuonef_EF_mu10_MSonly_EFFS_L1ZDC[i] = trig_EF_trigmuonef_EF_mu10_MSonly_EFFS_L1ZDC->at(i);
       if((RunNumber>=193403&&RunNumber!=194287)){
   ...
   int muonPairs = 0;
   int mu_muid_npairs = TMath::Binomial(mu_muid_n,2);

   for(int i=0;i<mu_muid_n;i++){

     ///loop over all muon combinations 
     for(int imu2=i+1; imu2<mu_muid_n; imu2++){

       if( (mu_muid_charge->at(i)*mu_muid_charge->at(imu2))<0.0 ) mu_muid_invMassOS[muonPairs] = getInvMass(i,imu2);
       else mu_muid_invMassOS[muonPairs] = -99999.0;
       if( (mu_muid_charge->at(i)*mu_muid_charge->at(imu2))>0.0 ) mu_muid_invMassSS[muonPairs] = getInvMass(i,imu2);
       else mu_muid_invMassSS[muonPairs] = -99999.0;
       ++muonPairs;
