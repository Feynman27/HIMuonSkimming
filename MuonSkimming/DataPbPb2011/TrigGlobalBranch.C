...
float TrigGlobalBranch::caloMissEz(){
   ...
   for(int i=0;i<TotalEt_n;i++){
	...
	float eta = TotalEt_layer_eta->at(i);
	float theta = TMath::ATan(2.0*TMath::Exp(-1.0*eta));
	float et = 0;
	...
	if (fabs(eta)>3.2) continue;
	missingEz = tan(theta) != 0.0 ? et/tan(theta) : 0.0; 


   }

	missingEz = missingEz/1000.;
	return missingEz;
}
/////////////////////////////////
...
void TrigGlobalBranch::Set_Trig_and_Global_Branches( TTree *MyTree)
{
   ...
   MyTree->Branch("caloMEz"          , &caloMEz, "caloMEz/F"              );
   MyTree->Branch("metEta"           , &metEta,   "metEta/F"              );
   MyTree->Branch("metPhi"           , &metPhi,   "metPhi/F"              );
...
float TrigGlobalBranch::AssignCentrality(float fcal_Et){
  ...
  float centralityTemp = -1.;
  	  if(fcal_Et >3.8) return centralityTemp;
	  else if(fcal_Et > 2.8038) {
...
/*  for(int itrig=0;itrig<N_L1_TRIGS_TAP;itrig++){
  ...
  for(int itrig=0;itrig<N_EF_TRIGS;itrig++){
    ...
    for (int i=0;i<EFsize;i++){
      if (trig_EF_passedPhysics->at(i) == EF_Trigs_id[itrig] ) EF_Trigs[itrig]=1;
       ...
       ///Get eta,phi of MET vector 	
       vMET.SetXYZ(caloMEx,caloMEy,caloMEz);
       metEta = vMET.Eta();
       metPhi = vMET.Phi();
  ...
  //Vertex info. the 0th vertex is always the primary one.
   ...
   if( Centrality < 0.) {
	std::cout << "WARNING.No centrality class found." << std::endl;
   }
